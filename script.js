  ...
  otherProcessContent.innerHTML = `
    ...
    <section class="ideas-storyboard-section memory-early-section" aria-label="记忆褶皱最初分镜草稿">
      ...
      <div class="memory-draft-frame-layout">
        ...
        <div class="memory-early-frames"><div class="memory-early-frames-head"><strong>部分关键帧</strong><span>11 张 · 滚动浏览，点击放大</span></div><div class="memory-early-frames-scroll" role="region" aria-label="记忆褶皱部分关键帧，可滚动浏览" tabindex="0">${Array.from({length:11},(_,i)=>`<a href="./assets/memory-early-keyframe-${String(i+1).padStart(2,'0')}.webp" target="_blank" rel="noopener" aria-label="放大查看《记忆褶皱》关键帧 ${i+1}"><img src="./assets/memory-early-keyframe-${String(i+1).padStart(2,'0')}.webp" alt="《记忆褶皱》部分关键帧 ${i+1}" loading="lazy" /><span>${String(i+1).padStart(2,'0')}</span></a>`).join('')}</div></div>
      </div>
    </section>
